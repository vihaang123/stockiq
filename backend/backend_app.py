...
def buy_stock(body: BuyRequest, username: str = Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        if existing:
            ...
            conn.execute(
                "UPDATE portfolio SET qty=?, avg_price=?, updated_at=datetime('now') WHERE username=? AND symbol=?",
...
def sell_stock(body: SellRequest, username: str = Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        if new_qty == 0:
            ...
            conn.execute(
                "UPDATE portfolio SET qty=?, updated_at=datetime('now') WHERE username=? AND symbol=?",
...
def get_transactions(
    ...
    q = (
        "SELECT id, username, symbol, type, qty, price, total, created_at "
...
def get_history(days: int = 120, username: str = Depends(get_current_user)):
    with get_db() as conn:
        rows = conn.execute(
            "SELECT date, value FROM portfolio_history WHERE username=? ORDER BY date ASC LIMIT ?",
            (username, days),
        ).fetchall()
...
def save_simulation(body: SimulationRequest, username: str = Depends(get_current_user)):
    ...
    with get_db() as conn:
        conn.execute(
            ...
            "VALUES (?,?,?,?,?,?,?,?)",
            (sim_id, username, body.symbol, body.amount, body.qty, body.price, body.before_score, body.after_score),
        )
